...
SC-005 claim — "one `platform.yaml` reproduces the system on a fresh environment with no manual
...
## What it does
`docker compose run --rm platform-validate` runs, inside a clean container, the full loop:
`platform install → platform up → platform status → change-value → platform apply → status`
against a `platform.yaml` whose component sources are pinned git refs (fetched at build), so the
container is reproducible from the one authority alone.

## DGX endpoint — pick one (deliberate sub-decision, test both ways)
Per research D2 the DGX is an *external* dependency `platform` health-checks, not starts.
- `DGX_MODE=real` — container reaches the real `192.0.2.10:8001`; exercises the real
  external-dependency path. Point `platform.yaml` at a different IP to also prove SC-004.
- `DGX_MODE=stub` — a stub endpoint stands in; assert `platform status` reports the *real* DGX
...
`/speckit.implement` (they depend on the `platform` CLI, which 001 builds). Marked here so the
