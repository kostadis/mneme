...
- [X] T022 [US1] **SC-002 verified 2026-06-24.** CampaignGenerator is the only repo where mneme externalized constants — its *.py logic is clean (zero hardcoded infra literals; only docstring/help examples remain, by decision). The 5 constants live in the single authority (mneme.yaml) + CG's wiring template. The other components needed no constant removal: sovereign/private libs keep their own (dgxlib's endpoint = dgx-fun's, etc.), mempalace/rpg-lib are env/CLI-driven, gm-assistant is markdown.
